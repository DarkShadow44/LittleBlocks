...
public class PacketLib {
    ...
    public static void registerClientPacketHandlers() {
        ...
        handler.getPacketHandler(PacketIds.LOGIN)
            .registerClientExecutor(CommandLib.SETTINGS, new ClientPacketLittleBlocksLoginExecutor());

        handler.getPacketHandler(PacketIds.UPDATE)
            .registerClientExecutor(CommandLib.UPDATE_CLIENT, new ClientBlockChangeExecutor());

        handler.getPacketHandler(PacketIds.ENTITY)
            .registerClientExecutor(CommandLib.ENTITY_COLLECTION, new ClientLittleCollectionExecutor());

        handler.getPacketHandler(PacketIds.PLAYER)
            .registerClientExecutor(CommandLib.COPIER_MESSAGE, new ClientCopierNotifyExecutor());
    ...
    public static void registerPacketHandlers() {
        ...
        playerHandler.registerServerExecutor(CommandLib.WAND_SWITCH, new PacketLittleWandSwitchExecutor());

        handler.registerPacketHandler(PacketIds.PLAYER, playerHandler);

        PacketLittleBlockHandler littleBlockHandler = new PacketLittleBlockHandler();
        littleBlockHandler.registerServerExecutor(CommandLib.BLOCK_ACTIVATED, new PacketLittleBlockActivatedExecutor());
        littleBlockHandler.registerServerExecutor(CommandLib.BLOCK_CLICKED, new PacketLittleBlockClickedExecutor());

        handler.registerPacketHandler(PacketIds.LOGIN, new PacketLoginHandler());

        handler.registerPacketHandler(PacketIds.TILE, littleBlockHandler);

        handler.registerPacketHandler(PacketIds.ENTITY, new PacketLittleBlockCollectionHandler());
    ...
    @SideOnly(Side.CLIENT)
    public static void blockActivated(World world, EntityPlayer entityplayer, int x, int y, int z, int q, float a,
        float b, float c) {
    ...
    public static void sendBlockPlace(World world, EntityPlayer entityplayer, int x, int y, int z, int side, float hitX,
        float hitY, float hitZ) {
        PacketLittleBlock placePacket = new PacketLittleBlock(
            x,
            y,
            z,
            side,
    ...
    public static void sendItemUse(World world, EntityPlayer entityplayer, int x, int y, int z, int side,
        ItemStack itemstack) {
    ...
    public static void sendBlockChange(World world, EntityPlayer entityplayer, int x, int y, int z) {
        ...
        PacketHelper.sendToPlayer(changePacket, (EntityPlayerMP) entityplayer);
    }
    ...
    public static void sendWandChange(World world, EntityPlayer entityplayer, int actionID) {
        ...
        PacketHelper.sendToPlayer(wandPacket, (EntityPlayerMP) entityplayer);
    }
