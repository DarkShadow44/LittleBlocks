...
public class LittleBlocksRenderer implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (block == ConfigurationLib.littleChunk) {
            ...
            if (isSolidPass) {
                return this.littleSolidBlocks.renderLittleBlocks(world, x, y, z, 0);
            } else {
                return this.littleAlphaBlocks.renderLittleBlocks(world, x, y, z, 1);
            }
        }
        return false;
    }
    ...
    private static void updateBlocks(LittleBlocksLittleRenderer blocks, Block littleBlock, int[] coords) {
        ...
        if (littleBlock != null) {
            if (littleBlock.getMaterial() != Material.air) {
                blocks.addLittleBlockToRender(littleBlock, coords[0], coords[1], coords[2]);
                blockAdded = true;
            }
        }
        if (!blockAdded) {
            blocks.removeLittleBlock(littleBlock, coords[0], coords[1], coords[2]);
        }
    }
