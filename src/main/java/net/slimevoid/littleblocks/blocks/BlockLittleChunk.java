...
public class BlockLittleChunk extends BlockContainer {
    ...
    public BlockLittleChunk(int id, Class<? extends TileEntity> clazz, Material material, float hardness,
        boolean selfNotify) {
        super(material);
        this.clazz = clazz;
    ...
    public Item getItem(World world, int x, int y, int z) {
        ...
        if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
            ...
            Block pickedBlock = tilelb.getBlock(xSelected, ySelected, zSelected);
    ...
    public int getDamageValue(World world, int x, int y, int z) {
        ...
        if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
            TileEntityLittleChunk tilelb = (TileEntityLittleChunk) tileentity;
            Block picked = tilelb.getBlock(xSelected, ySelected, zSelected);
            if (picked != null && picked != Blocks.air) {
                int xx = (x << 3) + xSelected;
                int yy = (y << 3) + ySelected;
                int zz = (z << 3) + zSelected;
                return picked.getDamageValue((World) tilelb.getLittleWorld(), xx, yy, zz);
            }
        }
        return 0;
    }
    ...
    public boolean removedByPlayer(World world, EntityPlayer entityplayer, int x, int y, int z) {
        ...
        if (id == ConfigurationLib.littleChunk) {
            ...
            EntityItemLittleBlocksCollection collection = new EntityItemLittleBlocksCollection(
                world,
                x,
                y,
                z,
                new ItemStack(ConfigurationLib.littleChunk));
            if (!tile.isEmpty()) {
                if (FMLCommonHandler.instance()
                    .getSide() == Side.CLIENT
                    && FMLClientHandler.instance()
                        .getClient().playerController.isInCreativeMode()) {
                            ...
                            for (int y1 = 0; y1 < tile.size; y1++) {
                                for (int z1 = 0; z1 < tile.size; z1++) {
                                    ...
                                    if (blockId != Blocks.air) {
                                        ItemStack itemToDrop = this
                                            .dropLittleBlockAsNormalBlock(world, x, y, z, blockId, contentMeta);
                                        if (itemToDrop != null) {
    ...
    private ItemStack dropLittleBlockAsNormalBlock(World world, int x, int y, int z, Block blockId, int metaData) {
        boolean dropsBlocks = blockId.getDrops(world, x, y, z, metaData, 0)
            .size() > 0 ? true : false;
        if (dropsBlocks) {
            Item idDropped = blockId.getItemDropped(metaData, world.rand, 0);
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int localSide,
        float hitX, float hitY, float hitZ) {
        // System.out.println("Activated");
        if (world.isRemote) {
            if (this.canPlayerPlaceBlockOrUseItem(world, entityplayer)) {
                try {
                    BlockUtil.getLittleController()
                        .onPlayerRightClickFirst(
                            entityplayer,
    ...
    private boolean canPlayerPlaceBlockOrUseItem(World world, EntityPlayer entityplayer) {
        ...
        if (denyPlacement) {
            ChatHelper.addColouredMessageToPlayer(entityplayer, EnumChatFormatting.BOLD, placementMessage);
            return false;
        }
        return true;
    }

    public void onServerBlockActivated(World world, EntityPlayer entityplayer, int x, int y, int z, int side,
        float hitX, float hitY, float hitZ) {
    ...
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer entityplayer) {
        if (world.isRemote) {
            BlockUtil.getLittleController()
                .clickBlock((x << 3) + xSelected, (y << 3) + ySelected, (z << 3) + zSelected, side);
        }
    }
    ...
    public void onServerBlockClicked(World world, int x, int y, int z, int side, EntityPlayer entityplayer) {
        if (entityplayer instanceof EntityPlayerMP) {
            ...
            if (!FMLCommonHandler.instance()
                ...
                BlockUtil.getLittleItemManager(player, world)
                    .onBlockClicked(x, y, z, side);
            } else {
    ...
    public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 player, Vec3 view) {
        ...
        returns = CollisionRayTrace.rayTraceLittleBlocks(this, player, view, x, y, z, returns, tile, isFluid);
        ...
        if (!returns.isEmpty()) {
            ...
            for (MovingObjectPosition ret : returns) {
                ...
                if (retBlock != Blocks.air) {
                    ...
                    if (isLiquid && isFluid) {
                        isLiquid = !(retBlock instanceof BlockStaticLiquid
