...
public enum EnumWandAction {
    ...
    public static void setNextActionForPlayer(EntityPlayer entityplayer) {
        ...
        ChatHelper.addColouredMessageToPlayer(
            ...
            "Little Wand now in " + nextAction.actionDescription);
    }
    ...
    public static boolean doWandAction(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        ...
        if (playerWandAction.equals(EnumWandAction.COPY_LB)) {
            return doCopyLB(itemstack, entityplayer, world, x, y, z, side, hitX, hitY, hitZ);
        }
        if (playerWandAction.equals(EnumWandAction.ROTATE_LB)) {
            return doRotateLB(itemstack, entityplayer, world, x, y, z, side, hitX, hitY, hitZ);
        }
        if (playerWandAction.equals(EnumWandAction.PLACE_LB)) {
            return doPlaceLB(itemstack, entityplayer, world, x, y, z, side, hitX, hitY, hitZ);
        }
        if (playerWandAction.equals(EnumWandAction.DESTROY_LB)) {
            return doDestroyLB(itemstack, entityplayer, world, x, y, z, side, hitX, hitY, hitZ);
        }
        return false;
    }

    private static boolean doRotateLB(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        int side, float hitX, float hitY, float hitZ) {
        if (world.getBlock(x, y, z) == ConfigurationLib.littleChunk) {
            ((BlockLittleChunk) ConfigurationLib.littleChunk).rotateLittleChunk(world, x, y, z, ForgeDirection.UP);
            return true;
        }
        return false;
    }

    private static boolean doPlaceLB(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        int side, float hitX, float hitY, float hitZ) {
        Block blockID = world.getBlock(x, y, z);
        if (blockID != ConfigurationLib.littleChunk) {
            if (!blockID.isReplaceable(world, x, y, z) || blockID instanceof IFluidBlock) {
                if (side == 0) {
                    --y;
                }
            ...
            if (blockID.getMaterial() == Material.air || blockID.isAir(world, x, y, z)
                || blockID.isReplaceable(world, x, y, z) && !(world.getBlock(x, y, z) instanceof IFluidBlock)) {
                world.setBlock(x, y, z, ConfigurationLib.littleChunk);
                ...
                if (newtile != null) {
                    ...
                    FMLCommonHandler.instance()
                        .getFMLLogger()
                        .warn("Could not initialize LittleChunk");
                }
            }
            return true;
        }
    ...
    private static boolean doCopyLB(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        int side, float hitX, float hitY, float hitZ) {
        if (((EntityPlayerMP) entityplayer).capabilities.isCreativeMode) {
            ...
            if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
                try {
                    ...
                    selectedLittleTiles.put(entityplayer, (TileEntityLittleChunk) tileentity);
                    tileLock.writeUnlock();
                } catch (InterruptedException e) {
                    LoggerLittleBlocks.getInstance("ItemLittleBlocksCopier")
                        .writeStackTrace(e);
                }
                return true;
                ...
                try {
                    ...
                    if (selectedLittleTile != null) {
                        ...
                        world.setBlock(xx, yy, zz, ConfigurationLib.littleChunk);
                        TileEntity newtile = world.getTileEntity(xx, yy, zz);
                            ...
                            for (int x1 = 0; x1 < ConfigurationLib.littleBlocksSize; x1++) {
                                for (int y1 = 0; y1 < ConfigurationLib.littleBlocksSize; y1++) {
                                    for (int z1 = 0; z1 < ConfigurationLib.littleBlocksSize; z1++) {
                                        if (oldtile.getBlock(x1, y1, z1) != Blocks.air) {
                                            Block blockId = oldtile.getBlock(x1, y1, z1);
                                            int metadata = oldtile.getBlockMetadata(x1, y1, z1);
                                            newtilelb.setBlockIDWithMetadata(x1, y1, z1, blockId, metadata);
                                            TileEntity oldLittleTile = oldtile.getChunkTileEntity(x1, y1, z1);
                                            if (oldLittleTile != null) {
                                                newtilelb.setTileEntity(x1, y1, z1, oldLittleTile);
                                            }
                                        }
                                    }
                                }
    ...
    private static boolean doDestroyLB(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        int side, float hitX, float hitY, float hitZ) {
        Block block = world.getBlock(x, y, z);
        if (block == ConfigurationLib.littleChunk) {
            ConfigurationLib.littleChunk.removedByPlayer(world, entityplayer, x, y, z, true);
