...
public class LittleBlocksHelper implements ISlimevoidHelper {
    ...
    public Block getBlock(World world, int x, int y, int z) {
        if (world != null) {
            return getWorld(world, x, y, z).getBlock(x, y, z);
        }
        return Blocks.air;
    }
    ...
    public TileEntity getBlockTileEntity(IBlockAccess world, int x, int y, int z) {
        if (world != null) {
            ...
            if (newWorld != null) {
                TileEntity tileentity = newWorld.getTileEntity(x, y, z);
                return tileentity;
            }
        }
        return null;
    ...
    private boolean isLittleBlock(IBlockAccess world, int x, int y, int z) {
        ...
        if (world.getTileEntity(x >> 3, y >> 3, z >> 3) instanceof ILittleBlocks) {
            return true;
        }
        return false;
    }
    ...
    public boolean isUseableByPlayer(World world, EntityPlayer player, int xCoord, int yCoord, int zCoord, double xDiff,
        ...
        if (isLittleBlock(world, xCoord, yCoord, zCoord)) {
            return player.getDistanceSq((xCoord / size) + xDiff, (yCoord / size) + yDiff, (zCoord / size) + zDiff)
                <= distance;
        }
        return false;
    }
    ...
    public boolean isLadder(IBlockAccess world, int x, int y, int z, EntityLivingBase entity) {
        ...
        if (((World) world).checkChunksExist(
            MathHelper.floor_double(minX),
            ...
            // is 0
            minX = MathHelper.floor_double((minX - MathHelper.floor_double(minX)) * tile.size)
                + ((MathHelper.floor_double(minX) - x) * tile.size);
            minY = MathHelper.floor_double((minY - MathHelper.floor_double(minY)) * tile.size)
                + ((MathHelper.floor_double(minY) - y) * tile.size);
            minZ = MathHelper.floor_double((minZ - MathHelper.floor_double(minZ)) * tile.size)
                + ((MathHelper.floor_double(minZ) - z) * tile.size);
            maxX = MathHelper.floor_double((maxX - MathHelper.floor_double(maxX)) * tile.size)
                + ((MathHelper.floor_double(maxX) - x) * tile.size);
            maxZ = MathHelper.floor_double((maxZ - MathHelper.floor_double(maxZ)) * tile.size)
            ...
            for (int littleX = (int) minX; littleX <= maxX; littleX++) {
                for (int littleZ = (int) minZ; littleZ <= maxZ; littleZ++) {
                    Block block = tile.getBlock(littleX, (int) minY, littleZ);
                    if (block != null && block.getMaterial() != Material.air) {
                        int xx = (x << 3) + littleX, yy = (y << 3) + (int) minY, zz = (z << 3) + littleZ;
                        if (block.isLadder((World) tile.getLittleWorld(), xx, yy, zz, entity)) {
                            return true;
                        }
                    }
                }
