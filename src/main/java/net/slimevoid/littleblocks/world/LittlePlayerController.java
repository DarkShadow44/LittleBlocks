...
public class LittlePlayerController extends PlayerControllerMP {
    ...
    public static void clickBlockCreative(Minecraft client, LittlePlayerController controller, int x, int y, int z,
        int side) {
        if (!((World) LittleBlocks.proxy.getLittleWorld(client.theWorld, false))
            .extinguishFire(client.thePlayer, x, y, z, side)) {
            controller.onPlayerDestroyBlock(x, y, z, side);
        }
    }

    public void onPlayerRightClickFirst(EntityPlayer entityplayer, World world, ItemStack itemstack, int x, int y,
        int z, int sumside, float hitX, float hitY, float hitZ) {
        boolean flag = true;
        ...
        if (itemstack.stackSize == 0) {
            ...
            FMLClientHandler.instance()
                .getClient().entityRenderer.itemRenderer.resetEquippedProgress();
        }

        if (flag) {
            ItemStack itemstack1 = entityplayer.inventory.getCurrentItem();
            if (itemstack1 != null && this.sendUseItem(entityplayer, world, itemstack1)) {
                FMLClientHandler.instance()
                    .getClient().entityRenderer.itemRenderer.resetEquippedProgress2();
            }
        }
    }
    ...
    @Override
    public boolean onPlayerRightClick(EntityPlayer entityplayer, World world, ItemStack itemstack, int x, int y, int z,
        int side, Vec3 hitAt) {
        ...
        if (itemstack != null && itemstack.getItem() != null
            && itemstack.getItem()
                .onItemUseFirst(itemstack, entityplayer, world, x, y, z, side, xOffset, yOffset, zOffset)) {
            return true;
        }

        if (!entityplayer.isSneaking() || (entityplayer.getHeldItem() == null || entityplayer.getHeldItem()
            .getItem()
            .doesSneakBypassUse/* shouldPassSneakingClickToBlock */(world, x, y, z, entityplayer))) {
            flag = world.getBlock(x, y, z)
                .onBlockActivated(world, x, y, z, entityplayer, side, xOffset, yOffset, zOffset);
        }

        if (!flag && itemstack != null && itemstack.getItem() instanceof ItemBlock) {
            ItemBlock itemblock = (ItemBlock) itemstack.getItem();

            if (!itemblock.func_150936_a/* canPlaceItemBlockOnSide */(world, x, y, z, side, entityplayer, itemstack)) {
                return false;
            }
        }

        // System.out.println("Controller: " + world);
        PacketLib.sendBlockPlace(world, entityplayer, x, y, z, side, xOffset, yOffset, zOffset);
        ...
        if (flag) {
            ...
            boolean placedOrUsed = itemstack
                .tryPlaceItemIntoWorld(entityplayer, world, x, y, z, side, xOffset, yOffset, zOffset);
            itemstack.setItemDamage(damage);
            itemstack.stackSize = stackSize;
            return placedOrUsed;
        } else {
            if (!itemstack.tryPlaceItemIntoWorld(entityplayer, world, x, y, z, side, xOffset, yOffset, zOffset)) {
                return false;
            }
            if (itemstack.stackSize <= 0) {
    ...
    public void clickBlock(int x, int y, int z, int side) {
        World littleWorld = (World) LittleBlocks.proxy.getLittleWorld(mc.theWorld, false);
        if (!BlockUtil.isLittleChunk(littleWorld, x, y, z)) {
            return;
        }
        if (this.currentGameType.isCreative()) {
            PacketLib.sendBlockClick(x, y, z, side);
            clickBlockCreative(this.mc, this, x, y, z, side);
        } else {
            PacketLib.sendBlockClick(x, y, z, side);
            Block block = littleWorld.getBlock(x, y, z);
            boolean flag = block.getMaterial() != Material.air;
            if (flag) {
                block.onBlockClicked(littleWorld, x, y, z, this.mc.thePlayer);
    ...
    public boolean onPlayerDestroyBlock(int x, int y, int z, int side) {
        ...
        if (stack != null && stack.getItem() != null
            && stack.getItem()
                .onBlockStartBreak(stack, x, y, z, this.mc.thePlayer)) {
            return false;
        }
        if (this.currentGameType.isCreative() && this.mc.thePlayer.getHeldItem() != null
            ...
            World littleWorld = (World) LittleBlocks.proxy.getLittleWorld(this.mc.theWorld, false);
            Block littleBlock = littleWorld.getBlock(x, y, z);

            if (littleBlock.getMaterial() == Material.air) {
                return false;
            } else {
                littleWorld.playAuxSFX(
                    2001,
                    x,
                    y,
                    z,
                    Block.getIdFromBlock(ConfigurationLib.littleChunk/*
                                                                      * littleBlock)
                ...
                if (this.mc.thePlayer.getHeldItem() != null && this.mc.thePlayer.getHeldItem()
                    ...
                    if (EnumWandAction.getWandAction()
                        .equals(EnumWandAction.DESTROY_LB)) {
                        littleWorld.setBlockToAir(x, y, z);
                        blockIsRemoved = true;
                    }
                }
                if (blockIsRemoved) {
                    littleBlock.onBlockDestroyedByPlayer(littleWorld, x, y, z, metadata);
                }
                ...
                if (!this.currentGameType.isCreative()) {
                    ...
                    if (itemstack != null) {
                        itemstack
                            .func_150999_a/* onBlockDestroyed */(littleWorld, littleBlock, x, y, z, this.mc.thePlayer);
