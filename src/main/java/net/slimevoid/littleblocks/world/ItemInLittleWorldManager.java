...
public class ItemInLittleWorldManager extends ItemInWorldManager {
    ...
    public ItemInLittleWorldManager(World world, EntityPlayerMP entityplayer) {
        super((World) LittleBlocks.proxy.getLittleWorld(world, false));
    ...
    public void onBlockClicked(int x, int y, int z, int side) {
        PlayerInteractEvent event = ForgeEventFactory
            .onPlayerInteract(thisPlayerMP, Action.LEFT_CLICK_BLOCK, x, y, z, side, theWorld);
        if (event.isCanceled()) {
            PacketLib.sendBlockChange(this.theWorld, this.thisPlayerMP, x, y, z);
            return;
        }
        if (this.isCreative()) {
            if (!this.theWorld.extinguishFire(this.thisPlayerMP, x, y, z, side)) {
    ...
    public boolean tryHarvestBlock(int x, int y, int z) {
        ...
        if (event.isCanceled()) {
            ...
            if (stack != null && stack.getItem()
                .onBlockStartBreak(stack, x, y, z, thisPlayerMP)) {
                return false;
            }
            Block block = this.theWorld.getBlock(x, y, z);
            int blockId = Block.getIdFromBlock(block);
            int metadata = this.theWorld.getBlockMetadata(x, y, z);
            this.theWorld.playAuxSFXAtEntity(
                this.thisPlayerMP,
                2001,
                x,
                y,
            ...
            if (this.isCreative()) {
                ...
                if (playerHeldItem != null) {
                    playerHeldItem
                        .func_150999_a/* onBlockDestroyed */(this.theWorld, block, x, y, z, this.thisPlayerMP);
                }

                blockHarvested = this.removeBlock(x, y, z);
                if (blockHarvested && canHarvest) {
                    block.harvestBlock(this.theWorld, this.thisPlayerMP, x, y, z, metadata);
                }
            }
    ...
    private boolean removeBlock(int x, int y, int z, boolean canHarvest) {
        ...
        Block littleBlock = this.theWorld.getBlock(x, y, z);
        int metadata = this.theWorld.getBlockMetadata(x, y, z);
        ...
        if (this.thisPlayerMP.getHeldItem() != null && this.thisPlayerMP.getHeldItem()
            ...
            if (EnumWandAction.forceDestroy(this.thisPlayerMP)) {
                ...
                if (!blockIsRemoved) {
                    ...
                    littleBlock.dropBlockAsItem(this.theWorld, x, y, z, metadata, fortune);
                }
                blockIsRemoved = true;
            }
        }
        ...
        if (blockIsRemoved) {
            littleBlock.onBlockDestroyedByPlayer(this.theWorld, x, y, z, metadata);
        }

        return blockIsRemoved;
    }

    public boolean activateBlockOrUseItem(EntityPlayer entityplayer, World world, ItemStack itemstack, int x, int y,
        int z, int side, float hitX, float hitY, float hitZ) {
        PlayerInteractEvent event = ForgeEventFactory
            .onPlayerInteract(entityplayer, Action.RIGHT_CLICK_BLOCK, x, y, z, side, world);
        if (event.isCanceled()) {
            PacketLib.sendBlockChange(this.theWorld, this.thisPlayerMP, x, y, z);
            return false;
        }
        ...
        if (item != null && item.onItemUseFirst(itemstack, entityplayer, world, x, y, z, side, hitX, hitY, hitZ)) {
            if (itemstack.stackSize <= 0) ForgeEventFactory.onPlayerDestroyItem(thisPlayerMP, itemstack);
            return true;
        }
        ...
        if (!useBlock) {
            useBlock = entityplayer.getHeldItem()
                .getItem()
                .doesSneakBypassUse(world, x, y, z, entityplayer);
        }
        boolean result = false;

        if (useBlock) {
            if (event.useBlock != Event.Result.DENY) {
                result = block.onBlockActivated(world, x, y, z, entityplayer, side, hitX, hitY, hitZ);
            } else {
                PacketLib.sendBlockChange(this.theWorld, this.thisPlayerMP, x, y, z);
        ...
        if (itemstack != null && !result && event.useItem != Event.Result.DENY) {
            ...
            result = itemstack.tryPlaceItemIntoWorld(entityplayer, world, x, y, z, side, hitX, hitY, hitZ);
            if (isCreative()) {
